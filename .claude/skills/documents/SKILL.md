...
### Branding
The invoice is templated from the owner's preferences — `brand_color` (hex accent on the header band, table header and footer), `invoice_template` (`classic` light header vs `modern` full-colour band), `invoice_footer` (a line like delivery terms or a UPI handle), and a logo.
- "put my logo on the bills" → the owner sends the image, then call `set_shop_logo` (it uses the photo they just sent).
...
- The deck has real charts (top items, payment mix, daily trend), a GST-slab breakup, stock-health table, a velocity-based reorder plan, an expiry watch (FEFO) and written insights. It is themed with the shop's brand colour and logo.
